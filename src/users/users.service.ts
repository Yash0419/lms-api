...
export class UsersService {
 ...
 async remove(id: number) {
  ...
  private async checkIfEmailExists(email: string, id?: number) {
    const doesEmailExists = await this.prisma.user.findFirst({
      where: { email },
    });
    if (doesEmailExists) {
      if (id && doesEmailExists.id !== id) {
        throw new BadRequestException(`User with ${email} already exixts`);
      } else if (!id) {
        throw new BadRequestException(`User with ${email} already exixts`);
      }
    }
  ...
  private async checkIfMobileExists(phone: string, id?: number) {
    const doesMobileExists = await this.prisma.user.findFirst({
      where: { phone },
    });
    if (doesMobileExists) {
      if (id && doesMobileExists.id !== id) {
        throw new BadRequestException(`User with ${phone} already exixts`);
      } else if (!id) {
        throw new BadRequestException(`User with ${phone} already exixts`);
      }
    }
